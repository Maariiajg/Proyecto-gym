    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <h2 class="font-black text-3xl text-white tracking-tighter uppercase">
                Modificar <span class="text-indigo-500 italic">Operador</span>
            </h2>
            <span class="px-3 py-1 bg-slate-900 border border-white/10 rounded-full text-[10px] font-black text-slate-500 uppercase tracking-widest">{{ $user->name }}</span>
        </div>
    </x-slot>
    ...
    <div class="py-12 min-h-screen">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-10">
            <div class="glass p-10 sm:p-14 rounded-[3rem] border border-white/5 shadow-2xl relative overflow-hidden group">
                <div class="absolute -bottom-24 -left-24 w-80 h-80 bg-purple-500/5 rounded-full blur-3xl group-hover:scale-125 transition-transform duration-700"></div>
                
                <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-8 relative z-10">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div class="space-y-3">
                            ...
                            <x-text-input id="name" name="name" type="text" class="w-full" :value="old('name', $user->name)" required autofocus />
                        ...
                        <div class="space-y-3">
                            <label for="password" class="block text-[10px] font-black text-indigo-400 uppercase tracking-widest">Nueva Contraseña <span class="text-slate-600">(Opcional)</span></label>
                            <x-text-input id="password" name="password" type="password" class="w-full" autocomplete="new-password" placeholder="••••••••" />
                            <x-input-error class="mt-2 text-[10px] font-black text-rose-500 uppercase" :messages="$errors->get('password')" />
                        </div>

                        <div class="space-y-3">
                            <label for="password_confirmation" class="block text-[10px] font-black text-indigo-400 uppercase tracking-widest">Confirmar Nueva</label>
                            <x-text-input id="password_confirmation" name="password_confirmation" type="password" class="w-full" placeholder="••••••••" />
                        ...
                        <div class="space-y-3">
                            ...
                            <select id="role" name="role" class="w-full px-6 py-4 glass bg-slate-900/50 border-white/5 text-white focus:border-indigo-500/50 focus:ring-indigo-500/50 rounded-2xl shadow-inner transition-all font-bold text-sm appearance-none cursor-pointer" required>
                                @foreach($roles as $role)
                                    <option value="{{ $role->name }}" {{ old('role', $userRole) == $role->name ? 'selected' : '' }} class="bg-slate-900">
